...
- **localhost限定・無認証・dev-mode単一プロセス**: 内部PoCツールであり本番SaaS基盤ではないため。`app.run(host="127.0.0.1", ...)` で固定し、`0.0.0.0` へのバインドは変更禁止（APIキーがプロセス内にのみ存在するため、外部公開する理由がない）。
...
- **出力先**: `poc/out/webapp/<timestamp>_<safe_name>.web.json`（`timestamp` はマイクロ秒粒度）。`batch_verify.py` のキャッシュ（`poc/out/` 直下 `<sname>.web.json`）とディレクトリごと分離されており、命名・キャッシュロジックへの相互影響はない。
